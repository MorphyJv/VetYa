...
export default function VetCalendarClient({
    ...
    const getTypeColor = (type: string) => {
        switch (type) {
            ...
            case "checkup": return "bg-teal-100 text-teal-700";
        ...
        <div className="flex flex-col lg:flex-row min-h-[600px]">
            ...
            <div className="flex-1 p-6 lg:border-r border-[var(--vy-neutral-200)]">
                <div className="flex items-center justify-between mb-8">
                    ...
                    <div className="flex gap-2">
                        <button
                            ...
                            className="w-10 h-10 flex items-center justify-center rounded-xl bg-[var(--vy-neutral-100)] text-[var(--vy-neutral-600)] hover:bg-[var(--vy-neutral-200)] transition-colors"
                        >
                            ←
                        </button>
                        <button
                ...
                <div className="grid grid-cols-7 gap-px bg-[var(--vy-neutral-200)] border border-[var(--vy-neutral-200)] rounded-2xl overflow-hidden">
                    ...
                    {Array.from({ length: daysInMonth }).map((_, i) => {
                        ...
                        return (
                            <button
                                ...
                                className={`bg-white min-h-24 p-2 relative flex flex-col items-start hover:bg-teal-50/40 transition-colors outline-none
                  ...
                  ${isSelected ? "bg-teal-600 text-white" : "text-[var(--vy-neutral-700)]"}`}>
                                    {day}
                                </span>
            ...
            <div className="w-full lg:w-96 bg-[var(--vy-neutral-50)] p-6 flex flex-col h-full">
                ...
                <div className="flex-1 overflow-y-auto space-y-4 mb-6">
                    {selectedDateEvents.length === 0 ? (
                        ...
                        selectedDateEvents.map(ev => (
                            <div key={ev.id} className={`p-4 bg-white rounded-2xl border ${ev.completed ? "opacity-60" : "border-[var(--vy-neutral-200)] shadow-sm"} transition-all`}>
                                <div className="flex justify-between items-start gap-4">
                                    <div className="flex-1">
                                        ...
                                        <h4 className={`font-bold text-[var(--vy-neutral-900)] leading-tight ${ev.completed ? "line-through" : ""}`}>
                                            {ev.title}
                                        </h4>
                                        {ev.pet?.name && (
                                    ...
                                    <button
                                        ...
                                        className={`w-8 h-8 flex items-center justify-center rounded-full border transition-colors shrink-0 ${ev.completed ? "bg-teal-500 border-teal-500 text-white" : "border-[var(--vy-neutral-300)] text-transparent hover:border-teal-500"}`}
                                    >
                                        ✓
                                    </button>
                                </div>
                ...
                <Link
                    ...
                    className="w-full flex items-center gap-2 py-2.5 px-4 rounded-xl border border-[var(--vy-neutral-200)] bg-white text-sm font-semibold text-[var(--vy-neutral-700)] hover:bg-teal-50 hover:border-teal-300 hover:text-teal-700 transition-all mb-3"
                >
                ...
                <motion.button
                    ...
                    className="w-full py-3 rounded-xl bg-teal-600 text-white font-semibold text-sm hover:bg-teal-700 transition-colors shadow-md disabled:opacity-50 disabled:cursor-not-allowed"
                >
                    {pets.length === 0 ? "Registra un paciente primero" : "Agregar Evento"}
                </motion.button>
            </div>
                ...
                {showModal && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                        ...
                        <motion.div
                            ...
                            className="bg-white p-6 rounded-3xl shadow-xl w-full max-w-md relative z-10"
                        >
                            <h3 className="text-xl font-bold text-[var(--vy-neutral-900)] mb-6">Nuevo Evento — Clínica</h3>
                            ...
                            <form onSubmit={handleAddEvent} className="space-y-4">
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        className="w-full px-4 py-3 rounded-xl border border-[var(--vy-neutral-300)] text-sm focus:ring-2 focus:ring-teal-500 outline-none"
                                    />
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <select
                                            name="pet_id"
                                            required
                                            className="w-full px-4 py-3 rounded-xl border border-[var(--vy-neutral-300)] bg-white text-sm focus:ring-2 focus:ring-teal-500 outline-none"
                                        >
                                ...
                                <div className="pt-4 flex justify-end gap-3">
                                    <button
                                        type="button"
                                        onClick={() => setShowModal(false)}
                                        className="px-5 py-2.5 rounded-xl text-sm font-medium text-[var(--vy-neutral-600)] hover:bg-[var(--vy-neutral-100)] transition-colors"
                                    >
                                        Cancelar
                                    </button>
                                    <button
                                        type="submit"
                                        disabled={loading}
                                        className="px-6 py-2.5 rounded-xl bg-teal-600 text-white text-sm font-semibold hover:bg-teal-700 shadow-md disabled:opacity-50"
                                    >
